...
def _infer_pip_size(symbol: str, cfg: dict[str, Any]) -> float:
    ...
    Prefer explicit per-symbol config maps if present. Otherwise use standard FX
    convention: JPY crosses use 0.01, other pairs use 0.0001.
    """
...
def _fix_spread_atr_units(df: pd.DataFrame, symbol: str, cfg: dict[str, Any]) -> pd.DataFrame:
    ...
    CSV was produced before the prepare_mt5_data.py fix. Formula:
        spread_atr = spread_points * spread_pips_per_point * pip_size / atr_14
    """
    ...
    pip_size = _infer_pip_size(symbol, cfg)
    spread_price = spread_points * _spread_pips_per_point(cfg) * pip_size
...
def _max_spread_pips_for_symbol(cfg: dict[str, Any], symbol: str) -> float | None:
    ...
    if points not in (None, ''):
        return float(points) * _spread_pips_per_point(cfg)
    return None
...
def _force_high_spread_rows_to_no_trade(
    ...
    spread_pips = spread_points * _spread_pips_per_point(cfg)
    ...
    for col in ('selected_side_target', 'side_target', 'trade_side_target'):
        if col in out.columns:
            ...
            # caused the high-spread filter to fail on USDJPY. Only write the numeric
