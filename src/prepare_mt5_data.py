...
def _infer_pip_size(symbol: str, cfg: dict[str, Any]) -> float:
    ...
    FX convention: JPY crosses use 0.01, other pairs use 0.0001.
    """
...
def fix_spread_atr_units(df: pd.DataFrame, symbol: str, cfg: dict[str, Any]) -> pd.DataFrame:
    ...
    ATR units. For JPY pairs this is around 1000x too large because one broker
    point is 0.001 price units when spread_pips_per_point=0.1 and pip_size=0.01.

    Correct formula:
        spread_pips = spread_points * spread_pips_per_point
        atr_pips = atr_14 / pip_size
    ...
    spread_price = spread_points * spread_pips_per_point * pip_size
