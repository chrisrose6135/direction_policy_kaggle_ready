...
def train_symbol(symbol: str, cfg: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    # Fit scaler on training-period raw rows only, then transform the full frame.
    # IGNORE targets (-1) from the strong-setup data preparation are skipped as
    # supervised endpoints by prepare_direction_arrays, but remain in the raw
    # dataframe so valid neighbouring sequences still have realistic history.
    ...
    if n < 100:
        raise RuntimeError(
            f'{symbol}: not enough labelled sequence rows for direction training after IGNORE filtering: '
    ...
    if len(train_idx) == 0 or len(val_idx) == 0:
        raise RuntimeError(
            ...
            f'val_start={val_start}, effective_embargo={embargo}'
        )
