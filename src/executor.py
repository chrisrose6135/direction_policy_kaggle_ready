...
def build_order_request(symbol: str, direction: str, cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    if direction == "BUY":
        ...
        sl = price - float(orders.get("stop_loss_pips", (cfg.get("labels", {}) or {}).get("stop_loss_pips", 5.0))) * pip
        tp = price + float(orders.get("take_profit_pips", (cfg.get("labels", {}) or {}).get("take_profit_pips", 6.0))) * pip
    elif direction == "SELL":
