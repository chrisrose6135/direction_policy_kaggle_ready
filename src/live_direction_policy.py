...
def evaluate_symbol(symbol: str, cfg: dict[str, Any], bundle: tuple, *, mode: str, data_source: str) -> dict[str, Any]:
    ...
    feat, meta = latest_processed_features(symbol, cfg)
    if len(feat) == 0:
    ...
    x, latest_row = _live_sequence(feat, cfg, scaler, feature_columns)

    with torch.no_grad():
        outputs = model(x.to(device))
        probs = direction_probabilities_from_outputs(outputs).cpu().numpy()[0]
        trade_probability = float(outputs.get('trade_probability', torch.tensor([float(probs[0] + probs[2])], device=x.device)).detach().cpu().view(-1)[0].item())
        side_sell_probability = outputs.get('side_sell_probability')
        side_buy_probability = outputs.get('side_buy_probability')
        buy_edge_output = outputs.get('buy_edge_pips')
        sell_edge_output = outputs.get('sell_edge_pips')
        side_sell_probability = None if side_sell_probability is None else float(side_sell_probability.detach().cpu().view(-1)[0].item())
        side_buy_probability = None if side_buy_probability is None else float(side_buy_probability.detach().cpu().view(-1)[0].item())
        buy_edge_pips = None if buy_edge_output is None else float(buy_edge_output.detach().cpu().view(-1)[0].item())
        sell_edge_pips = None if sell_edge_output is None else float(sell_edge_output.detach().cpu().view(-1)[0].item())
